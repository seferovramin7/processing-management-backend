...
public class CardService implements CardFactory {
    ...
    public ResponseEntity<ResponseDto> storeCard(CardSaveDto dto) {
        ...
        if (user == null) {
            return new ResponseEntity<>(ResponseDto.builder().result(Result.USER_NOT_FOUND).build(), HttpStatus.NOT_FOUND);
        }
        ...
        if (existingCard != null) {
            return new ResponseEntity<>(ResponseDto.builder().result(Result.ALREADY_EXIST).build(), HttpStatus.BAD_REQUEST);
        }

        cardRepository.save(cardEntity);
        log.debug("Card successfully stored for user");

        return new ResponseEntity<>(ResponseDto.builder().result(Result.SUCCESS).build(), HttpStatus.OK);
